...
type SupportedDuration = z.infer<
...
/*
 * These simulate an external calendar and database.
 */
...
async function findAvailableSlots(
    ...
    const slots: ConsultationSlot[] = startTimes
        .filter(
            (startsAt) =>
                !occupiedStartTimes.has(startsAt),
        )
        .map((startsAt) => ({
            id: createSlotId(
                startsAt,
...
async function createBooking(
    ...
    /*
     * The selected slot must belong to the latest
     * availability result.
     */
    const slot = availability.slots.find(
        (candidate) =>
            candidate.id === args.slot_id,
    );
    ...
    /*
     * Return the original booking when the same
     * logical write is retried.
     */
    const existingBooking =
        bookingsByIdempotencyKey.get(
            idempotencyKey,
        );
    ...
    /*
     * Recheck external state immediately before
    ...
    /*
     * Simulate durable storage protected by
...
const bookConsultation = tool({
    ...
    description:
        "Create a consultation booking using a slot from the latest availability result. This changes external state and requires explicit human approval before execution.",
...
- Do not ask for conversational confirmation before calling book_consultation. The application provides a separate approval checkpoint before execution.
...
`.trim());
    ...
    try {
        while (true) {
            ...
            /*
             * A write tool with needsApproval pauses
             * the run and creates an interruption.
             */
            while (result.interruptions.length > 0) {
                for (
                    ...
                    /*
                     * Reject any unexpected approval request.
                     */
                    if (
                        interruption.name !==
                        "book_consultation"
                    ...
                    /*
                     * The application verifies the model's
                     * proposed arguments before showing approval.
                     */
                    if (!args || !slot) {
                        result.state.reject(interruption, {
                            message:
                ...
                /*
                 * Continue the exact interrupted run.
                 * Do not restart using history here.
                 */
                result = await run(
                    consultationAgent,
                    result.state,
